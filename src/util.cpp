...
int nAnonymizePlexusAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "Plexus" is a composite category enabling all Plexus-related debug output
            if (ptrCategory->count(string("Plexus"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Plexus
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Plexus
// Mac: ~/Library/Application Support/Plexus
// Unix: ~/.Plexus
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "plexus.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override plexus.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Plexusd.pid"));
