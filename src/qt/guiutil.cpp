...
{
    // return if URI is not valid or is no Plexus: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::Plexus, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert Plexus:// to Plexus:
    //
    //    Cannot handle this later, because Plexus:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::Plexus, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open Plexus.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Plexus.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Plexus.lnk
...
{
    return GetAutostartDir() / "Plexus.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Plexus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Plexus\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Plexus app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Plexus app to startup item list
...
void restoreWindowGeometry(const QString& strSetting, const QSize& defaultSize, QWidget* parent)
{
    ...
    QSize size = settings.value(strSetting + "Size").toSize();
